...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "telefy/teleswap-subgraph") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const V1_DATA_QUERY = gql`
  query teleswap($date: Int!, $date2: Int!) {
    current: teleswap(id: "1") {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
    oneDay: teleswapHistoricalDatas(
    ...
    twoDay: teleswapHistoricalDatas(
...
export const GLOBAL_CHART = gql`
  query teleswapDayDatas($startTime: Int!, $skip: Int!) {
    teleswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query teleswapFactories {
      teleswapFactories(
